...
def _lite_edit_is_executable(
    ...
    if _lite_edit_status(edit).startswith("label_only_"):
        return False
...
def _prepare_lite_execution_request(
    request: RevisionRequest,
    doc_items: Optional[List[RevisionReviewItem]],
) -> Tuple[RevisionRequest, List[str], List[str], List[str]]:
    """Remove every marker-only operation from the request that writes the draft."""

    execution_request, skipped_edit_ids = _filter_lite_execution_edits(request, doc_items)
    authoritative = doc_items if doc_items is not None else request.review_items
    unresolved_ids = {
        item.item_id
        for item in authoritative
    ...
    pause_policy_ids.update(
        str(pause.item_id or "").strip()
        for pause in request.pause_adjustments
        if str(pause.item_id or "").strip()
    )
    all_label_only_ids = list(
        dict.fromkeys(
            [
                *skipped_edit_ids,
                *sorted(pause_policy_ids),
                *sorted(unresolved_ids),
            ]
        )
    )
    return (
        replace(execution_request, pause_adjustments=[]),
        all_label_only_ids,
        sorted(unresolved_ids),
        sorted(pause_policy_ids),
    )
...
def _lite_pause_offset_before(
    ...
    *,
    ...
    # Pause requests are source-time labels in Lite. They never offset the timeline.
    _ = (point, pauses, include_at_point)
    return 0.0
...
def _validate_spoken_cut_alignment(
    ...
    for edit in request.edits:
        ...
        ) or lite_pause_change_is_label_only(
            source_kind,
...
def _validate_lite_audio_timing_sources(
    ...
    for item in authoritative:
        ...
        if lite_pause_change_is_label_only(item.kind, item.source_text):
            ...
            elif str(item.kind or "").strip().casefold() == "semantic_pause_adjustment":
                problems.extend(_semantic_pause_asr_problems(request, item))
            else:
                problems.append(
                    f"Lite audio timing {item.item_id}: pause change has no ASR-resolved "
...
def _marker_items(
    marker_type: Any,
    execution_request: RevisionRequest,
    doc_items: Optional[List[RevisionReviewItem]],
...
def _validate_lite_content(
    ...
    *,
    ...
    working_audio_bindings: Optional[List[Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    errors: List[str] = []
    ...
    segment_receipt_rows = list(segment_receipts or [])
    if pauses:
        errors.append(
            "Lite draft must not contain executable pause_adjustments; "
    ...
    if abs(saved_duration - expected_timeline_duration) > 0.01:
        errors.append(
            "Lite draft duration changed: expected unchanged source duration "
...
def execute_lite_revision_request(
    ...
    *,
    ...
    # Validate audio evidence before filtering marker-only edits. Pause timing
    # remains ASR-bound even though Lite never writes a hold or changes duration.
    marker_source_request = request
    _validate_lite_audio_timing_sources(request, doc_items)
    _validate_spoken_cut_alignment(request, doc_items)
    (
        request,
        label_only_item_ids,
        unresolved_label_only_item_ids,
        pause_label_only_item_ids,
    ) = _prepare_lite_execution_request(request, doc_items)
    if request.pause_adjustments:
        raise RuntimeError(
            "Lite execution policy error: pause_adjustments remained after label-only filtering."
        )
    ...
    if not mock_media and not request.project.source_pairs:
        for field in ("source_video", "source_audio", "replacement_audio"):
            expected = getattr(request.project, field + "_sha256", "")
            if expected and _file_sha256(getattr(request.project, field)) != expected:
                raise ValueError("Working audio source material changed before localization")
    ...
    working_audio_bindings: List[Dict[str, Any]] = []
    working_audio_sync_reports: List[Dict[str, Any]] = []
    ...
    try:
        ...
        if pair_mode:
            ...
            for pair in source_pairs:
                ...
                # Preserve the original in the library even when no timeline
                # segment should play it. A1/A2 will select working material.
                project.script.add_material(pair.source_audio_material)
                pair.working_audio_material = pair.source_audio_material
                pair.working_audio_duration = pair.source_audio_duration
                if pair.audio_mode == "replace_original":
        ...
        if pair_mode:
            # Each pair owns a working source. Kept A1 clips and audible
            # deleted A2 trace clips use local source clocks, while their
            ...
            for keep_start, keep_end in keep_audio_windows:
                for pair, local_start, local_end, global_start in _pair_intersections(
                    ...
                    keep_start,
                    keep_end,
                    duration_field="working_audio_duration",
                ):
                    _add_audio_segment(
                        project,
                        draft,
                        pair.working_audio_material,
            ...
            audio_material = source_pairs[0].working_audio_material
            audio_duration = total_duration
            source_audio_duration = max(
                pair.offset + pair.working_audio_duration for pair in source_pairs
            )
        ...
        markers, marker_plan, marker_warnings = _marker_items(
            ...
            doc_items,
            timeline_duration,
            marker_source_request=marker_source_request,
        )
        ...
        if pause_receipts:
            raise RuntimeError(
                "Lite execution policy error: pause receipts were created for label-only items."
            )
        ...
        for variant_name, content in variants:
            variant_validation = _validate_lite_content(
                ...
                source_video_material_id=str(getattr(video_material, "material_id", "")),
                ...
                source_pair_audio_durations=(
                    [pair.working_audio_duration for pair in source_pairs] if pair_mode else None
                ),
            )

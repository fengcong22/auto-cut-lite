...
`auto-cut-high-school-history-lite` 是“高中历史 Auto-Cut 精简版”入口。它设置
`workflow_mode=lite`，继续复用统一路由和能力模块，但使用固定轨道、只切不删、原时长
不变的非破坏性草稿规则。未指定模式时仍使用全能力版。
...
| `auto-cut-high-school-history-lite` | 高中历史精简版。固定 V1-V4/A1-A2 轨道，删除意见只切不删，原文标签顶部显示 2 秒，素材不做动画和自动缩放，原视频时长不变。 | “用高中历史 Auto-Cut 精简版”、“精简版 Auto-Cut”、“只切不删” |

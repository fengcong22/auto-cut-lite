"""
Auto-Cut - 全面功能演示 (Comprehensive Showcase)
...
skill_candidates = [
    ...
    os.path.join(current_dir, ".agent", "skills", "Auto-Cut"),
    os.path.join(current_dir, "Auto-Cut", ".agent", "skills", "Auto-Cut"),
    os.path.abspath(".agent/skills/Auto-Cut"),
]
skill_root = next(
...
if not skill_root:
    raise ImportError("Could not locate Auto-Cut skill root for full_feature_showcase.")
...
def run_full_showcase():
    ...
    pip_seg = project.add_media_safe(
        VIDEO_PATH, start_time=pip_start, duration=pip_dur, track_name="OverlayTrack"
    )

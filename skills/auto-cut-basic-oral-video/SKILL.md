...
description: Summarize, reuse, or apply the repository's basic oral-video style for JianYing editable drafts. Use when the user asks for `基础口播视频`, basic oral-video style, standard口播视频, or wants the same剪辑节奏, BGM matching logic, flower-text template sourcing, sound-effect sourcing, lower-center subtitle and flower-text layout, red-box safe-zone placement, flower intro/outro animation semantics, fixed `优设标题黑` flower font, and cooler-and-brighter portrait color style derived from `C2597_v37_bgm_cloud_funk`.
---
...
This rule set is repository-scoped. Keep it in this repository under `skills/auto-cut-basic-oral-video/` so future draft builds and revisions can call it directly when the user says `基础口播视频`.

This is the repo's default short oral-video style entrypoint. Its current concrete source style is derived from `C2597_v37_bgm_cloud_funk`.
...
Treat the current `基础口播视频` baseline, derived from `C2597_v37_bgm_cloud_funk`, as this combination:
...
Keep the pacing close to the C2597 cut map:
...
This skill is derived from the repository-local reconstruction and review artifacts for:

- `tmp/c2597_build_flowerfx.py`
- `tmp/c2597_rebuild_v36_native_temp_curve.py`
- `tmp/c2597_rebuild_v37_bgm_cloud.py`
- reference draft: `C2597_v37_bgm_cloud_funk`

If a future version intentionally departs from this style, create a new style skill rather than silently mutating this one.

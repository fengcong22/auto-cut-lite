...
`skills/` is the source of truth and `.codex/skills` is a refreshable repository-local copy.
Use `python scripts/install_repo_skills.py --list` to inspect the catalog without copying.
...
| `auto-cut-subject-pointer-onboarding` | 课程指向物学科建档。响应用户明确的建立、更新、补充、查看、绑定或改绑命令，也接收普通指向物门禁失败后的 same-task onboarding handoff；不从内容推断身份。 | Explicit subject-library commands or a pointer-targeting same-task handoff |
...
- “为当前项目建立高中历史指向物素材库” -> `auto-cut-subject-pointer-onboarding`
- “加小手/箭头/圈选指向老师标注的位置，位置要精准，大小按原视频参考素材，只给入点但要判断停留多久消失” -> `auto-cut-pointer-targeting` -> exact gate fails only: `auto-cut-subject-pointer-onboarding` -> fresh pointer gate + `auto-cut-revision-draft`

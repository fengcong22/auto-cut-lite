...
| `subject_evidence` | Required subject-pointer evidence is missing, incomplete, stale, or awaiting scale-reference confirmation. |
| `preview_approval` | A final subject-pointer preview needs explicit approval. |
...
1. Persist the durable wait before notification delivery. A resumable subject-pointer job does this with `review-job-wait-open`. An interactive-only blocker first posts the full blocking question in the originating Codex task.
...
## Subject-Pointer Waits

The four subject-pointer actions use the durable `subject_pointer_onboarding.json` sidecar:
...
`authorization` and `high_risk_confirmation` are interactive-only in this release. They never create or reuse the subject-pointer `external_wait`.

...
If the project is unbound or any profile, asset, hotspot, scale-reference, layout, ratio, preview, path, role, or hash evidence is missing or invalid, stop before overlays and draft writing, then open the same-task onboarding handoff defined in `auto-cut-subject-pointer-onboarding/references/handoff-contract.md`. Preserve the original pointer and source-ledger item IDs, requested role, current layout names, and project context. Never infer stage or subject from the review document, OCR, ASR, filename, title, layout, or media; ask only for the missing identity. Direct library requests still use the explicit create/update command, but an ordinary pointer gate failure continues through onboarding in this same task.

...
Use this repository-local skill as the final gate before reporting a review-driven JianYing draft as done. It does not perform the edits itself; it checks that the saved draft and reports prove the edits were executed and remain inspectable.
...
`currentCustomDraftPath` is read from `JianyingPro/User Data/Config/globalSetting`. A missing, ambiguous, non-existent, reparse-point, or mismatched path fails closed with `请在对方电脑创建与源电脑相同的剪映草稿路径：<path>`; the checker never silently selects a conventional default.

The recipient must use the same absolute path as the source computer. A first-use check that returns `configured_target_path_missing` or any other path error is a user action, not permission to choose a default directory.

All local video, audio, and image files used by the draft must be inside `Resources/local` or `Resources/audioAlg` before the material reference is written. The timeline remains editable and source media remains visible.

Deliver the saved directory unchanged:
...
The mirror has a mandatory source-stability gate. Before each copy attempt it
must observe the complete tree receipt (regular-file hashes, byte sizes, and
empty directories) unchanged for a continuous quiet window. It copies only to
a temporary sibling, verifies the temporary tree, then observes the source for
another continuous quiet window before atomically promoting the temporary
directory. A late source mutation discards only that temporary directory and
restarts from a fresh stable receipt; timeout or retry exhaustion fails closed
with no final target and no success receipt. The default unattended policy is
`quiet_window_seconds=6`, `poll_interval_seconds=1`,
`stability_timeout_seconds=120`, and `stability_retries=2`; callers may pass
these options to `draft-mirror-deliver`. Receipts must include the policy,
observation counts/timestamps, `source_stable_before_copy=true`,
`source_stable_after_copy=true`, `source_stable_at_promotion=true`, and the
actual retry count.

`capture_draft_tree_receipt` records the complete source tree and target tree, including empty directories. The external receipt is written outside both draft trees and binds `schema_version`, `source_tree_sha256`, `target_tree_sha256`, `tree_sha256`, `file_count`, `directory_count`, `byte_size`, and the job-input digest. The mirror is byte-for-byte and refuses overwrite, refuses a destination that already exists (`destination already exists`), reparse points, source/target overlap, source changes during copy, or any SHA-256 mismatch. It records the flags `json_rewritten`, `ui_invoked`, `native_editor_invoked`, and `portable_package_invoked` as `false`.

The old `portable_project_tool` package/import code may remain as historical/internal compatibility code, but it is not routed and is never acceptance evidence. The fixed absolute path and source tree/target tree receipt are the only delivery evidence.
...
- for complete-project delivery: fixed source/recipient path, external receipt path, source/target SHA-256 values, byte-for-byte counts, no-UI/no-JSON-rewrite flags, local material paths, non-file dependencies, and whether second-computer `target_open_verified` remains pending

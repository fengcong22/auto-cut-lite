    ...
    for start in starts:
        for candidate in (start, *start.parents):
            ...
            if (candidate / "pyproject.toml").is_file():
                return candidate
    raise RuntimeError("could not find a repository pyproject.toml")


def _registry_root(registry_root: str | os.PathLike[str] | None) -> Path:
    if registry_root is None:
        return _find_repository_root() / "data" / "subject-pointer-profiles.local"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage exact subject-pointer profiles.")
    ...
    for command_parser in (
        ...
        command_parser.add_argument(
            "--root", help="Registry root; defaults to repository-local data."
        )
        command_parser.add_argument(

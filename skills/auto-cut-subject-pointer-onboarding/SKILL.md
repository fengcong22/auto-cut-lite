---
name: auto-cut-subject-pointer-onboarding
description: "Use when the user explicitly asks to create, update, supplement, view, bind, or rebind a repository-local subject pointer library, or when pointer targeting opens a same-task onboarding handoff."
---
...
Build or maintain one pointer-material profile for one user-supplied `学段 + 学科`. Profiles live under `data/subject-pointer-profiles.local`. Direct entry runs only when the user explicitly requests a library operation. An ordinary pointer request enters only through the same-task onboarding handoff opened by `auto-cut-pointer-targeting` after its exact binding/profile gate fails.
...
- `为当前项目建立高中历史指向物素材库`: create or complete the named profile, then bind the current project after it becomes ready.
- `建立高中历史指向物素材库`: create or complete the profile without guessing a project binding.
- `更新高中历史指向物素材库`: repair stale evidence or add a missing layout reference.
- `查看当前有哪些学科指向物素材库`: list registered profiles.
- `查看高中历史指向物素材库`: inspect one exact named profile.
- `把当前项目绑定到高中历史素材库`: bind an existing ready profile.
- `将当前项目改绑为高中地理素材库`: explicitly replace an existing project binding.
...
```json
{
  "list-all": {
    ...
    "command": "python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py list --root <registry-root> --json"
  },
  "create": {
    "example": "建立高中历史指向物素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  },
  "update": {
    "example": "更新高中历史指向物素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  },
  "supplement": {
    "example": "补充高中历史指向物素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  },
  "view-profile": {
    "example": "查看高中历史指向物素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  },
  "bind": {
    "example": "把当前项目绑定到高中历史素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  },
  "rebind": {
    "example": "将当前项目改绑为高中地理素材库",
    "identity_policy": "ask_only_missing_explicit_identity_and_stop"
  }
}
```
...
```powershell
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py identity --stage-name <stage_name> --subject-name <subject_name> --json
```
...
```powershell
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py check --stage-id <stage_id> --subject-id <subject_id> --root <registry-root> --json
```
...
```powershell
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py migrate-hand-media --stage-id <stage_id> --subject-id <subject_id> --root <registry-root> --json
```
...
```powershell
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py register --input <intake.json> --root <registry-root> --json
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py check --stage-id <stage_id> --subject-id <subject_id> --root <registry-root> --json
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py list --root <registry-root> --json
python skills/auto-cut-subject-pointer-onboarding/scripts/profile_registry.py validate --root <registry-root> --json
```
...
```powershell
python skills/auto-cut-subject-pointer-onboarding/scripts/project_bindings.py bind --project-key <project_key> --project-path <project_path> --project-root <project_root> --stage-id <stage_id> --subject-id <subject_id> --root <registry-root> --json
```

For an explicitly confirmed rebind, repeat the same command with `--rebind`; the function-level equivalent is `allow_rebind=True`. A plain `建立...素材库` command without current-project binding wording creates the profile only. Project bindings are stored in `data/subject-pointer-profiles.local/project-bindings.json`.

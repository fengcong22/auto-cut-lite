...
class Exporter:
    ...
    def _find_jianying_path(self):
        ...
        if user_profile:
            paths.append(
                os.path.join(
                    user_profile,
                    r"AppData\Local\JianyingPro\Apps\JianyingPro.exe",
                )
            )
        for p in paths:
            if os.path.exists(p):
    ...
    def switch_to_home(self):
        ...
        close_btn = self.window.GroupControl(
    ...
    def open_draft(self, draft_name):
        ...
        for attempt in range(12):  # 延长到 24 秒等待首页渲染
            ...
            draft_text = self.window.TextControl(
                searchDepth=6, Compare=ControlFinder.desc_matcher(target_desc, exact=True)
            )
            ...
            def partial_matcher(control, depth):
                try:
                    return draft_name in control.Name
                except Exception:
                    return False
    ...
    def export(self, output_path, draft_name, resolution="1080P", framerate="30fps"):
        ...
        # 2. 点击顶部【导出】按钮
        export_btn = self.window.TextControl(
            searchDepth=5, Compare=ControlFinder.desc_matcher("MainWindowTitleBarExportBtn")
        )
        ...
        try:
            ...
            export_path_sib = self.window.TextControl(
                searchDepth=8, Compare=ControlFinder.desc_matcher("ExportPath")
            )
        ...
        if resolution:
            try:
                ...
                # 点击分辨率下拉框
                res_btn = self.window.TextControl(
                    searchDepth=10, Compare=ControlFinder.desc_matcher("ExportSharpnessInput")
                )
                if res_btn.Exists(1):
                    ...
                    if not res_item.Exists(0.5):
                        ...
                        res_item = self.window.TextControl(
                            searchDepth=5, Compare=ControlFinder.desc_matcher(target_val)
                        )
        ...
        # 5. 点击【导出】确认按钮
        confirm_btn = self.window.TextControl(
            searchDepth=8, Compare=ControlFinder.desc_matcher("ExportOkBtn")
        )
        ...
        while time.time() - start_time < 900:  # 最长 15 分钟
            close_btn = self.window.TextControl(
                searchDepth=8, Compare=ControlFinder.desc_matcher("ExportSucceedCloseBtn")
            )
            if close_btn.Exists(0):
                dprint("✅ 导出完成检测成功！")
                close_btn.Click(simulateMove=False)
        ...
        try:
            ...
            try:
                ...
                possible_dirs.extend(
                    [user_video_dir, user_desktop, os.path.join(user_video_dir, "Jianying")]
                )
            except Exception:
                pass
                ...
                for d in set(possible_dirs):
                    ...
                    for f in os.listdir(d):
                        if f.lower().endswith((".mp4", ".mov")) and (
                            safe_name in f or f in safe_name
                        ):
...
def run(draft_name, output_path):
    ...
    # 由于我们没改路径，这里简单提示
    dprint("⚠️ 注意: 视频已导出到剪映默认目录，请在剪映提示的文件夹中查找。")

...
Before execution, require the plugin manifest, runtime directory, isolated Python executable, and deployment report with `deployment_status=installed`. Stop if any is missing. Do not fall back to a source repository, another Auto-Cut checkout, system Python, or files copied from the source computer.

Run commands documented as `python scripts/<name>.py ...` from `$autoCutLiteRuntime`, using `$autoCutLitePython`. Paths documented as `data/...`, `schemas/...`, `presets/...`, or `tools/...` are relative to `$autoCutLiteRuntime`. Skill-local scripts are resolved under `$autoCutLiteRoot\skills\<skill-name>\scripts` and run with `$autoCutLitePython`.

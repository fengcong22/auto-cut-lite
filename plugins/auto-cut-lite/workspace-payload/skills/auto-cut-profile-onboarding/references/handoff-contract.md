...
Use this contract when `auto-cut-pointer-targeting` cannot validate the current project's exact binding and ready subject profile. The handoff continues the original pointer task through `auto-cut-profile-onboarding`; it is not a terminal instruction to start another request.
...
```json
{
  ...
  "artifact_payload": {
    ...
    "project": {
      "project_key": "topic-a-lesson-01",
      "draft_path": "<draft-root>/topic-a-lesson-01",
...
The command validates that closed envelope, canonicalizes `artifact_payload`, writes it as `pointer_profile_onboarding.json`, and derives the stored wait. The stored projection has the same `item_ids` and `input_digest`; it is output, not input:

```json
{
  "code": "awaiting_subject_profile",
  "phase": "pointer_profile_gate",
  "item_ids": ["source-item-id"],
  "input_digest": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
  "artifact": "pointer_profile_onboarding.json",
...
5. resolves the external wait with its current input and artifact digests, then reruns `pointer_profile_gate` from normal entry.

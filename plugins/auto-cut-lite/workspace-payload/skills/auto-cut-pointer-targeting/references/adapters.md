...
Read the current project from `%LOCALAPPDATA%\Auto-Cut\auto-cut-lite\pointer-profiles.local/project-bindings.json`, then read profiles only from the same packaged registry. `auto-cut-pointer-targeting` consumes the manually bound `profile_key` and a fresh `status=ready` receipt; it does not create, promote, repair, or automatically bind profiles. A direct library operation still requires an explicit create/update command. In ordinary pointer work, missing or invalid exact evidence opens the same-task onboarding handoff before draft writes; onboarding collects explicit user input and returns through a fresh gate.

Store this receipt under `review_items[*].evidence.subject_profile_receipt` and set `acceptance.require_pointer_profile_binding=true`:
...
```json
{
  "subject_profile_receipt": {
    ...
    "project_path": "<draft-root>/ProjectA_v01",
    "profile_key": "generic-course-profile",
    "profile_path": "<registry-root>/generic-course-profile/profile.json",
    "stage_id": "stage-a",
    "subject_id": "topic-a",
    "status": "ready",
    "asset_path": "<registry-root>/generic-course-profile/assets/aaaaaaaaaaaa-topic-a-pointer.png",
    ...
    "scale_reference_path": "<registry-root>/generic-course-profile/scale-references/bbbbbbbbbbbb-topic-a-map-reference.png",
    ...
    "scale_reference_id": "topic-a-map-lesson-12",
    ...
    "approved_preview_path": "<registry-root>/generic-course-profile/approved-previews/cccccccccccc-topic-a-approved-preview.png",

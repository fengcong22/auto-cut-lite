...
Use this packaged skill to turn loose course-review comments into precise editable JianYing pointer overlays. A pointer is any visual asset with an anchor that should land on a target: a fingertip, arrow head, circle center, magnifier focus, highlight center, or custom marker hotspot.
...
- `auto-cut-profile-onboarding`: load for an explicit library create/update/supplement/view/bind/rebind operation, or when this skill opens the required same-task onboarding handoff after an ordinary pointer gate fails.
...
Read [references/handoff-contract.md](../auto-cut-profile-onboarding/references/handoff-contract.md) and open a same-task onboarding handoff to `auto-cut-profile-onboarding` before draft writes whenever the exact gate above cannot pass. This handoff continues the current request; do not end with a command for the user to start another task.
...
For every course pointer revision request, set `acceptance.require_pointer_profile_binding=true`. Store the complete fresh result under `review_items[*].evidence.subject_profile_receipt`, including the absolute packaged `registry_root`. Strict validation treats every pointer item as binding-required even if the flag is omitted or false, so the flag is an explicit contract record, not an opt-out. The validator reloads the canonical packaged binding and profile; a self-declared `status=ready` value or alternate registry root is never sufficient.

...
The local transcript must contain alphanumeric content, transcript aliases must agree after normalization, and the transcript must prove every `must_keep` phrase. Every retained occurrence is represented by one positive `delete_hit` and exactly one entry in `delete_hit_adjudications`. Each receipt carries `classification=kept_recurrence`, the correct `occurrence_role`, `phrase`, independent `local_context`, `context_anchor`, `reason`, and an exact `hit` object with `phrase`, `text`, `start`, `end`, and `match_method`. It must match one reported hit exactly, use an allowed item-contract delete phrase, and lie outside all target cut windows. Missing, duplicate, tampered, overlapping, or unreported occurrences fail. A single legacy `delete_hit_adjudication` remains compatible only when there is exactly one positive hit. For `colored_span_delete`, the ordered item-contract `delete_phrases` define the allowed targets rather than the aggregate display phrase, and uncolored text between target spans is automatic `must_keep` evidence with its exact ASR window.

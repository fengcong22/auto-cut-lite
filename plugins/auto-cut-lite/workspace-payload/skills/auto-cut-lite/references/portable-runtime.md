...
The integrity check treats target-local asset indexes and sync outputs as mutable caches
(`data/*.local.csv`, `data/jy_cached_audio.csv`, refreshed cloud indexes, and `assets/jy_sync/**`).
All other managed runtime code and package files must match `PACKAGE-MANIFEST.json`; a drift is a
hard stop that requires redeployment.
